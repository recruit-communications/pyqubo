...
Summary of each integer encoding representing [0, n].

.. csv-table::
   :header: Encoding, Value, Constraint, #vars, Max abs. coeff of value
   :widths: 5, 4, 5, 5, 5

   :class:`UnaryEncInteger`, :math:`\sum_{i=1}^{n}x_{i}`, No constraint, :math:`n+1`, :math:`1`
   :class:`LogEncInteger`, :math:`\sum_{i=1}^{d}2^i x_{i}`, No constraint, :math:`\lceil\log_{2}n\rceil(=d)`, :math:`2^d`
   :class:`OneHotEncInteger`, :math:`\sum_{i=0}^{n}ix_{i}`, :math:`(\sum_{i=0}^{n}x_{i}-1)^2`, :math:`n`, :math:`n`
